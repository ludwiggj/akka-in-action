...
class RemoteLookupProxy(path: String) extends Actor with ActorLogging {
  ...
  def sendIdentifyRequest(): Unit = {
    ...
    println("identify: sendIdentifyRequest")
  ...
  def identify: Receive = {
    case ActorIdentity(`path`, Some(actor)) =>
      ...
      println("identify: switching to active state")
      context.become(active(actor))
      context.watch(actor)

    case ActorIdentity(`path`, None) =>
      log.error(s"identify: remote actor with path $path is not available.")
      println(s"identify: remote actor with path $path is not available.")
    ...
    case msg: Any =>
      ...
      println(s"identify: ignoring message $msg, remote actor is not ready yet.")
      // Try to send message again
      Thread.sleep((1 seconds).toMillis)
      log.info(s"identify: try again with message $msg")
      println(s"identify: try again with message $msg")
      self forward msg
  }
  ...
  def active(actor: ActorRef): Receive = {
    case Terminated(actorRef) =>
      ...
      println("active: actor $actorRef terminated.")
      println("active: switching to identify state")
    ...
    case msg: Any =>
      ...
      println(s"active: got message $msg")
      actor forward msg
      println(s"active: forwarded message $msg")
  }
}